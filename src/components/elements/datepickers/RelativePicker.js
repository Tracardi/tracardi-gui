...
export default function RelativePicker({ type, onDateSelect, datetime }) {
  ...
  const [periodType, setPeriodType] = useState(
    datetime?.delta?.type ? datetime.delta.type : defaultType
  );
  const [period, setPeriod] = useState(
    datetime?.delta?.value
      ? datetime.delta.value
      : 15 * (defaultType === "minus" ? -1 : 1)
  );
  const [periodEntity, setPeriodEntity] = useState(
    datetime?.delta?.entity ? datetime.delta.entity : "minute"
  );
  ...
  return (
    <div className="PeriodPicker">
      <div className="PeriodHeader">Select period</div>
      <div className="PeriodForm">
        <span
          style={{ margin: 10, fontSize: "1.5em", textTransform: "uppercase" }}
        >
          {defaultType}
        </span>
        ...
        >
          <MenuItem value={"minute"} selected>
            Minutes
          </MenuItem>
